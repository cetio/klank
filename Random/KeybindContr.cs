...
namespace Klank
{
    ...
    {
        private Dictionary<VirtualKeyCode, Func<object[], bool>> Binds = new Dictionary<VirtualKeyCode, Func<object[], bool>>();
        private Dictionary<Func<object[], bool>, object[]> Params = new Dictionary<Func<object[], bool>, object[]>();
        private Dictionary<VirtualKeyCode, int> Toggles = new Dictionary<VirtualKeyCode, int>();
        private List<Func<object[], bool>> FuncQueue = new List<Func<object[], bool>>();

        private Timer Timer = new Timer();

        [DllImport("user32.dll")]
        private static extern short GetAsyncKeyState(VirtualKeyCode vKey);
        ...
        private bool IsKeyDown(VirtualKeyCode key)
        {
        ...
        public bool Add(Func<object[], bool> func, VirtualKeyCode key, params object[] parameters)
        {
            Binds.Add(key, func);
        ...
        public bool Remove(VirtualKeyCode key)
        {
            if (Binds.ContainsKey(key))
            {
                _ = Params.Remove(Binds[key]);
                _ = Binds.Remove(key);
                _ = Toggles.Remove(key);
        ...
        {
            foreach (VirtualKeyCode key in Binds.Keys)
            {
                if (IsKeyDown(key))
                {
                    if (Toggles[key] >= 2)
